...
// Enqueue admin CSS
add_action( 'admin_enqueue_scripts', 'plugin\taxonomies\enqueue_styles' );

// Admin setup
add_action( 'admin_init', 'plugin\taxonomies\taxonomies_settings_flush_rewrite' );
add_action( 'add_meta_boxes', 'plugin\taxonomies\taxonomies_metabox' );
add_action( 'save_post', 'plugin\taxonomies\taxonomies_save_metabox' );
add_filter( 'post_updated_messages', 'plugin\taxonomies\taxonomies_update_messages' );

// Table columns
add_filter( 'manage_custom_taxonomies_posts_columns', 'plugin\taxonomies\custom_taxonomies_posts_columns' );
add_action( 'manage_custom_taxonomies_posts_custom_column', 'plugin\taxonomies\custom_taxonomies_posts_custom_columns', 10, 2 );
...
function taxonomies_metabox() {
	...
	add_meta_box(
		...
		'plugin\taxonomies\custom_taxonomies_metabox_content',
		array( 'custom_taxonomies' ),
		'advanced',
		'high'
	);
	...
	add_meta_box(
		...
		'plugin\taxonomies\custom_taxonomies_metabox_side',
		array( 'custom_taxonomies' ),
		'side',
		'low'
	);
...
function taxonomies_metabox_side( $post ) {
	?>
    <p><?php esc_html_e( 'For more info about each field please check', 'taxonomies' ); ?> <a
                href="https://developer.wordpress.org/reference/functions/register_post_type/"
                target="_blank"><?php esc_html_e( 'the official documentation.', 'taxonomies' ); ?></a></p>
	<?php
}
...
function custom_taxonomies_posts_custom_columns( $column_name, $id ) {
	...
	if ( $column_name === 'slug' ) {
		echo esc_html( get_post_meta( get_the_ID(), 'taxonomy_name', true ) );
	}
